...
/* Duty cycle helpers */
#define DUTY_50_PERCENT(period)         ((period) / 2U)
...
extern gpt_three_phase_instance_ctrl_t g_three_phase_ctrl;
extern const three_phase_cfg_t g_three_phase_cfg;
...
static three_phase_cfg_t g_three_phase_cfg_test;
static gpt_three_phase_instance_ctrl_t g_three_phase_ctrl_test;
...
static void test_setup_config (timer_mode_t mode, three_phase_buffer_mode_t buf_mode, uint32_t dead_time)
{
    ...
    memcpy(&g_three_phase_cfg_test, &g_three_phase_cfg, sizeof(three_phase_cfg_t));
    g_three_phase_cfg_test.buffer_mode = buf_mode;
    g_three_phase_cfg_test.p_timer_instance[0] = &g_timer_instances_test[0];
    g_three_phase_cfg_test.p_timer_instance[1] = &g_timer_instances_test[1];
    g_three_phase_cfg_test.p_timer_instance[2] = &g_timer_instances_test[2];
}

static fsp_err_t test_open_three_phase (void)
{
    memset(&g_three_phase_ctrl_test, 0, sizeof(g_three_phase_ctrl_test));

    return R_GPT_THREE_PHASE_Open(&g_three_phase_ctrl_test, &g_three_phase_cfg_test);
}

static void test_close_three_phase (void)
{
    R_GPT_THREE_PHASE_Stop(&g_three_phase_ctrl_test);
    R_GPT_THREE_PHASE_Close(&g_three_phase_ctrl_test);
}
...
static void comp_pwm_test_REQ_OM_01 (void)
{
    ...
    if (pass)
    {
        ...
        {
            pass &= verify_gtber2_single_buffer(&g_three_phase_ctrl_test, ch);
        }

        /* Verify mode is set correctly (GTCR.MD = 0xC for Mode 1) */
        uint32_t gtcr_md = (g_three_phase_ctrl_test.p_reg[THREE_PHASE_CHANNEL_U]->GTCR >> R_GPT0_GTCR_MD_Pos) & 0xFU;
        pass &= (gtcr_md == 0xCU);
...
static void comp_pwm_test_REQ_OM_03 (void)
{
    ...
    if (FSP_SUCCESS == err)
    {
        for (three_phase_channel_t ch = THREE_PHASE_CHANNEL_U; ch <= THREE_PHASE_CHANNEL_W; ch++)
        {
            pass &= verify_gtber2_double_buffer(&g_three_phase_ctrl_test, ch);

            /* Verify GTCCRF is initialized (double buffer register) */
            uint32_t gtccrf = g_three_phase_ctrl_test.p_reg[ch]->GTCCR[COMP_PWM_PRV_GTCCRF];
            pass &= (gtccrf != 0U);
        }
...
static void comp_pwm_test_REQ_OM_04 (void)
{
    ...
    if (pass)
    {
        ...
        err = R_GPT_THREE_PHASE_DutyCycleSet(&g_three_phase_ctrl_test, &duty);
        pass &= (FSP_SUCCESS == err);

        /* Short delay then verify GTCCRA received the value */
        R_BSP_SoftwareDelay(1U, BSP_DELAY_UNITS_MICROSECONDS);
        uint32_t gtccra = g_three_phase_ctrl_test.p_reg[THREE_PHASE_CHANNEL_U]->GTCCR[COMP_PWM_PRV_GTCCRA];
...
static void comp_pwm_test_REQ_DT_05 (void)
{
    ...
    if (pass)
    {
        ...
        uint32_t gtdvu = g_three_phase_ctrl_test.p_reg[THREE_PHASE_CHANNEL_U]->GTDVU;
...
static void comp_pwm_test_REQ_DT_06 (void)
{
    ...
    if (pass)
    {
        uint32_t gtpr  = g_three_phase_ctrl_test.p_reg[THREE_PHASE_CHANNEL_U]->GTPR;
        uint32_t gtdvu = g_three_phase_ctrl_test.p_reg[THREE_PHASE_CHANNEL_U]->GTDVU;
...
static void comp_pwm_test_REQ_DT_07 (void)
{
    ...
    if (pass)
    {
        ...
        g_three_phase_ctrl_test.p_reg[THREE_PHASE_CHANNEL_U]->GTDVU = new_dt;
        gtdvu = g_three_phase_ctrl_test.p_reg[THREE_PHASE_CHANNEL_U]->GTDVU;
        pass &= (gtdvu == new_dt);
...
static void comp_pwm_test_REQ_DT_08 (void)
{
    ...
    if (pass)
    {
        /* Verify GTDTCR.TDE is enabled (dead time output active) */
        uint32_t gtdtcr = g_three_phase_ctrl_test.p_reg[THREE_PHASE_CHANNEL_U]->GTDTCR;
        pass &= (gtdtcr != 0U);    /* TDE bit should be set when dead_time > 0 */

        /* Verify GTDVU is non-zero, providing non-overlapping guard */
        pass &= (g_three_phase_ctrl_test.p_reg[THREE_PHASE_CHANNEL_U]->GTDVU > 0U);
...
static void comp_pwm_test_REQ_DC_09 (void)
{
    ...
    if (pass)
    {
        err = R_GPT_THREE_PHASE_Start(&g_three_phase_ctrl_test);
        pass &= (FSP_SUCCESS == err);
        ...
        pass &= (g_three_phase_ctrl_test.p_reg[THREE_PHASE_CHANNEL_U]->GTCCR[COMP_PWM_PRV_GTCCRD] == period / 4U);
        pass &= (g_three_phase_ctrl_test.p_reg[THREE_PHASE_CHANNEL_V]->GTCCR[COMP_PWM_PRV_GTCCRD] == period / 2U);
        pass &= (g_three_phase_ctrl_test.p_reg[THREE_PHASE_CHANNEL_W]->GTCCR[COMP_PWM_PRV_GTCCRD] == (period * 3U) / 4U);
...
static void comp_pwm_test_REQ_DC_10 (void)
{
    ...
    if (pass)
    {
        ...
        uint32_t gtccrd = g_three_phase_ctrl_test.p_reg[THREE_PHASE_CHANNEL_U]->GTCCR[COMP_PWM_PRV_GTCCRD];
        pass &= (gtccrd >= period);
...
/*******************************************************************************************************************//**
 * @brief REQ-DC-12: Prevent 16-bit overflow on compare match value (RA2T1).
 **********************************************************************************************************************/
...
static void comp_pwm_test_REQ_SEC_17 (void)
{
    ...
    if (pass)
    {
        ...
        uint32_t prev_cnt = g_three_phase_ctrl_test.p_reg[THREE_PHASE_CHANNEL_U]->GTCNT;

        for (uint32_t sample = 0U; sample < 2000U; sample++)
        {
            uint32_t cnt = g_three_phase_ctrl_test.p_reg[THREE_PHASE_CHANNEL_U]->GTCNT;
