...
typedef struct s_gpt_gtior_setting
{
    union
    {
        ...
        struct
        {
            ...
            uint32_t cpscir : 1;       ///< Complementary PWM Mode Initial Output at Synchronous Clear Disable
            ...
            uint32_t oaeocd : 1;       ///< GTCCRA Compare Match Cycle End Output Invalidate
            uint32_t psye   : 1;       ///< PWM Synchronous Output Enable
...
typedef struct st_gpt_extended_cfg
{
    ...
    uint32_t compare_match_value[6];              ///< Storing compare match value for channels. The order of the register values is A-B-C-E-D-F.
    uint8_t  compare_match_status;                ///< Storing the compare match registers status. The order of the channel status is A-B-C-E-D-F.
